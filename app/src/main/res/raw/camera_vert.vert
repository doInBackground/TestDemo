...
//"varying"声明的变量,表示可变变量(易变变量),用于在顶点程序和片元程序之间传递值,在两个程序中分别声明同名的该类型变量后,一边赋值改变另一边也会变.
//"attribute"声明的变量,表示要从Java传来的变量,该类型只能定义在顶点程序中.
...
precision mediump float;//"precision"表示设置某个类型的精度(highp高(16位)/mediump中(10位)/lowp低(8位)). 此处表示所有"float"使用"mediump"精度.
