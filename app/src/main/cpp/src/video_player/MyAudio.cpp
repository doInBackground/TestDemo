...
MyAudio::MyAudio(MyPlaystatus *playstatus, int sample_rate, MyCallJava *callJava) {
...
void *decodeAudioPlay(void *data) {
    MyAudio *wlAudio = (MyAudio *) data;
...
void MyAudio::play() {
    ...
    pthread_create(&thread_play, NULL, decodeAudioPlay, this);
...
//播放   喇叭需要取出数据   ---》播放
void audioPcmBufferCallBack(SLAndroidSimpleBufferQueueItf bf, void *context) {
//    LOGE("-------->打印 ");
    MyAudio *wlAudio = (MyAudio *) context;
    if (wlAudio != NULL) {
...
//            audioPcmBufferCallBack 回调次数比较频繁    回调java
...
//    注册回调缓冲区 获取缓冲队列接口
    ...
    //缓冲接口回调
    (*pcmBufferQueue)->RegisterCallback(pcmBufferQueue, audioPcmBufferCallBack, this);
//    获取播放状态接口
    (*pcmPlayerPlay)->SetPlayState(pcmPlayerPlay, SL_PLAYSTATE_PLAYING);
    audioPcmBufferCallBack(pcmBufferQueue, this);
}
